...
class DefaultEventSeeder extends Seeder
{
    ...
    {
        // Clear existing default events (templates only, not school-year specific)
        DefaultEvent::whereNull('school_year')->delete();
        ...
        $events = [
            ...
            // July (month: 7) - Mid-Year events
            ['name' => 'Mid-Year Registration', 'month' => 7, 'order' => 1],
            ['name' => 'Beginning of Mid-Year Classes', 'month' => 7, 'order' => 2],
            
            // August (month: 8) - Mid-Year events
            ['name' => 'Mid-Year Final Examination', 'month' => 8, 'order' => 1],
            ['name' => 'Last Day of Submission of Mid-Year Grades', 'month' => 8, 'order' => 2],
        ];
        ...
        foreach ($events as $eventData) {
            DefaultEvent::create([
                ...
                'date' => null, // No date for templates
                'end_date' => null,
                'school_year' => null, // Template, not school-year specific
            ]);
        }

        $this->command->info('Seeded ' . count($events) . ' default academic events.');
    }
}